...
@("branch-name-1") | % { git fetch upstream; git checkout -b $_ upstream/$_; git merge upstream/master; Start-Process "chrome.exe" "https://github.com/{some-org}/{some-repo}/compare/$_...{some-fork}:$_?expand=1&title=Update%20with%20the%20latest%20master&body={some-body}" }; "git push --set-upstream {some-fork} $_" | clip;
